    ...
    loop {
        match receiver.recv() {
            ...
            Ok(Command::Update {
                ...
                store.get_mut(patch.id).map(|ticket| {
                    if let Some(t) = patch.title { ticket.title = t; }
                    if let Some(d) = patch.description { ticket.description = d; }
                    if let Some(s) = patch.status { ticket.status = s; }
                });
